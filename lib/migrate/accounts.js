...
async function insertAccounts() {
  ...
  accounts.forEach((acc) => {
    ...
    const data = {
      ...
      amount: acc.fin.balance * 10000 / getConversionRate(acc.bc.accountCurrency, new Date(CONFIG.START_DATE).toISOString().replace('T', ' ').replace('Z', '')
        .split(' ')[0]),
      transactionCurrency: acc.bc.accountCurrency,
      conversionRateNew: getConversionRate(acc.bc.accountCurrency, new Date(CONFIG.START_DATE).toISOString().replace('T', ' ').replace('Z', '')
        .split(' ')[0]),
      date: new Date(CONFIG.START_DATE).toISOString().replace('T', ' ').replace('Z', ''),
      transactionTypeID: 2,
      categoryID: 2,
      accountID: accId,
      status: 2,
