...
function insertTransactions() {
  ...
  const filtered = [
    transactions.find(transaction => transaction.fin.isExpense),
    transactions.find(transaction => transaction.fin.isTransfer),
    transactions.find(transaction => transaction.fin.isIncome)
  ];
...
function insertTransactionExpense(transaction) {
  ...
  const transactionName = transaction.note === 'Account balance update' ? 'Account balance update' : getTransactionName(transaction);
  ...
  const data = {
    ...
    amount: -transaction.amount * 10000 / getConversionRate(acc.accountCurrency, 'MDL'),
    ...
    status: 0,
    accountReference: 1,
    accountPairID: transaction.note === 'Account balance update' ? 3 : acc.accountsTableID,
...
function insertTransactionIncome(transaction) {
  ...
  const data = {
    ...
    date: new Date(transaction.date).toISOString().replace('T', ' ').replace('Z', ''),
    transactionTypeID: transaction.transaction_type + 2,
    categoryID: cat && cat.categoryTableID || 18,
    accountID: acc.accountsTableID,
    status: 0,
    accountReference: 1,
    accountPairID: transaction.note === 'Account balance update' ? 3 : 0,
    deletedTransaction: 6,
    transferGroupID: 0
  };
  const transactionInfo = database.insert('TRANSACTIONSTABLE', Object.values(data), Object.keys(data));
