...
function insertAccounts() {
  ...
  for (const acc of accounts) {
    ...
    columns = [
      ...
      'deletedTransaction', 'transferGroupID'
    ];
    values = [
      itemInfo.lastInsertROWID, acc.fin.balance,
      `'${acc.bc.accountCurrency}'`, 1.0,
      `'${new Date().toISOString().replace('T', ' ').replace('Z', '')}'`,
      2, 2, accId, 2, 3, accId, 6, 0
    ];
    const transactionInfo = database.insert('TRANSACTIONSTABLE', values, columns);
